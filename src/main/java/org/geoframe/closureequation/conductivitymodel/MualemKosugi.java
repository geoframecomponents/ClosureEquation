...
public class MualemKosugi extends ConductivityEquation {
	...
	public double k(double x, double y, int id, int element) {
		...
		if (saturationDegree < 1) {
			return super.closureEquation.getParameters().kappaSaturation[id] * Math.pow(this.saturationDegree, 0.5)
					* Math.pow((0.5 * Erf.erfc(Math.log(x / super.closureEquation.getParameters().par1[id])
							/ (Math.sqrt(2) * super.closureEquation.getParameters().par2[id])
							+ super.closureEquation.getParameters().par2[id] / Math.sqrt(2))), 2);
		} else {
